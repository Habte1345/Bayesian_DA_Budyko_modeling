...
# --- Define fixed time and file paths (USER MUST UPDATE THESE) ---
# NOTE: The paths are placeholders and must be updated to your local environment.
...
TWSA_PATH = r"C:\Users\hdagne1\Box\Dr.Mesfin Research\Data\GRACE\TWSA_Monthly_CAMELS.csv" # Not used, but included for completeness
...
# =====================================================================
# 1. HELPER FUNCTIONS for Streamflow I/O
# =====================================================================

def read_streamflow_file(file_path):
    """Reads a single CAMELS streamflow file and returns daily data."""
    try:
...
def import_streamflow_data(base_folder):
    """Imports and processes CAMELS daily streamflow data into monthly mean DataFrame."""
    all_data = []
    if not os.path.exists(base_folder):
        print(f"Warning: Streamflow base folder not found at {base_folder}")
        return pd.DataFrame()
...
# =====================================================================
# 2. MAIN DATA PROCESSING FUNCTION
# =====================================================================

def load_and_prepare_data() -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, 
                                     pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    """
    Loads, filters, aligns, and prepares all hydrologic and catchment data.
    
    Returns: P_df, PET_df, ET_obs_df, Qb_df, M_df, Slope_df, Q_nldas_df, Q_usgs_df
    """
    ...
    # Load and process USGS Streamflow
    ...
    # Convert USGS Q from cfs to mm/day
    area_km2_series = attrs['area_gages2'].loc[streamflow_data_all.columns]
    conversion_constant = 0.028316846 * 86400 * 1000 / 1e6 # cfs -> mm/day per km^2
    ...
    try:
        FluxData_all = xr.open_mfdataset(NLDAS_DATA_DIR + '/*.nc', concat_dim='time', combine='nested', 
    ...
    # --- Standardization and Common Basins ---
    attrs['gauge_id'] = attrs['gauge_id'].astype(str).str.strip().str.zfill(8)
    Q_USGS_monthly_full.columns = Q_USGS_monthly_full.columns.astype(str).str.strip().str.zfill(8)
    
    common_gauges = list(set(attrs['gauge_id']) & set(Q_USGS_monthly_full.columns))
    
    # Filter for the N_TOP_BASINS largest from the common set
    attrs_filtered = attrs[attrs['gauge_id'].isin(common_gauges)].copy()
    attrs_large = attrs_filtered.sort_values(by="area_gages2", ascending=False).head(N_TOP_BASINS).set_index('gauge_id')
    BASIN_IDS = attrs_large.index.tolist() 
    
    # --- NLDAS Extraction (Area-Weighted Mean) ---
    ...
    # --- Final Dataframe Assignment and Post-Processing ---
    Rainf_df = dfs['Rainf']
    ...
    # Q_USGS and NDVI filtered and aligned
    ...
    # Convert NLDAS Streamflow to mm/day
    ...
    # Calculate M (Vegetation Coverage)
    ...
    # Prepare Slope DF (Static value copied across the time index)
    ...
    # Final Return (8 aligned DataFrames)
